...
function Country({ countries }) {
  ...
  return (
    <div className={isDarkMode ? "Country-bg-dk" : "Country-bg"}>
      <button className={isDarkMode ? "Country-btn-dk" : "Country-btn"} onClick={() => navigate(-1)}><ArrowBackIcon/> Back</button>
      <div className="Country-info">
        <img className="Coutry-flag" src={country.flags[1]} />
        <div className="Country-details">
          ...
          <div className="Country-details-borders">
            <p><span>Border Countries: </span>{country.borders[0] ? 
            borderCountries.map(borderCountry => (
              <Link to={`/country/${borderCountry}`}>
                <button className={isDarkMode ? "BorderCountry-link-dk" : "BorderCountry-link"}>{borderCountry}</button>
              </Link>
            )) : 
            "N/A"}</p>
          </div>
