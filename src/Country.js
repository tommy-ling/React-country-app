...
function Country({ countries }) {
  ...
  return (
    <div>
    <button className="Country-btn" onClick={() => navigate(-1)}><ArrowBackIcon/> Back</button>
      ...
      <div className="Country-details">
        ...
        <div className="Country-details-borders">
          ...
          borderCountries.map(borderCountry => (
            <Link to={`/country/${borderCountry}`}><button className="BorderCountry-link">{borderCountry}</button></Link>
          )) : 
          "N/A"}</p>
        </div>
      </div>
